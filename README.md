...
|---|---|---|---|---|
| qwen2.5:3b-instruct (Ollama, CPU) | 0.733 | 0.653 | 24 608 мс | 38 455 мс |
| llama-3.3-70b-versatile (Groq) | 0.911 | 0.833 | 2 009 мс | 1 731 мс |

n=45, реальный прогон `python scripts/compare_models.py`. Разница в latency
(~12x) даже больше гипотезы — она объясняется не только размером модели, но и
железом: Qwen на этой машине идёт на CPU без CUDA (см. «Честные ограничения»),
Groq — на собственных LPU-чипах. Разница в accuracy (0.733 → 0.911) и macro
F1 (0.653 → 0.833) — тоже реальная, но её нельзя приписать целиком модели:
Llama на порядок крупнее (70B против 3B) и лучше инструктирована, при том что
RAG-контекст и промпт идентичны для обеих.

**p95 ниже среднего у Groq — не опечатка.** Свободный тир (12000 TPM) дважды
за 45 запросов упёрся в 429 — `src/llama_groq_client.py` делает retry с
паузой по `x-ratelimit-reset-tokens`, и эта пауза попадает в измеренную
latency (2 сообщения из 45 — 22.5с и 6.1с вместо обычных 1.2-1.8с). Именно
поэтому среднее (2009мс) выше p95 (1731мс): выбросы тянут среднее вверх, но
не входят в 95-й перцентиль. "Чистая" latency инференса — медиана 1417мс.
